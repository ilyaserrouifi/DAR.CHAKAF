...
// en base64 (data URI) directement dans la colonne TEXT de la DB.
...
async function uploadImage(imageUrl, type = 'gallery') {
    ...
    // Cas 3 : chemin interne déjà servi par l'application
    if (/^\/(assets|uploads)\//i.test(imageUrl)) {
        return {
            url: imageUrl,
            filename: `${type}_${Date.now()}`,
            path: null
...
async function deleteImage() {
    // Rien à supprimer physiquement : soit c'est un lien externe ou asset local,
